...
class NotificationSchedulerTest {
  ...
  void processNotifications_shouldProcessNotifications() {
    ...
    Set<String> notificationIds = Set.of(schedule1Id.toString(), schedule2Id.toString());
    ...
    given(redisNotificationService.getNotificationsToSend(any(LocalTime.class)))
        .willReturn(notificationIds);
    given(notificationScheduleRepository.findById(schedule1Id))
        .willReturn(Optional.of(schedule1));
    given(notificationScheduleRepository.findById(schedule2Id))
        .willReturn(Optional.of(schedule2));
    ...
    verify(notificationScheduleRepository, times(1))
        .findById(schedule1Id);
    verify(notificationScheduleRepository, times(1))
        .findById(schedule2Id);
    ...
    NotificationScheduleDto scheduleDto1;
    NotificationScheduleDto scheduleDto2;

    if (capturedDtos.get(0).getId() == schedule1Id) {
      scheduleDto1 = capturedDtos.get(0);
      scheduleDto2 = capturedDtos.get(1);
    } else {
      scheduleDto1 = capturedDtos.get(1);
      scheduleDto2 = capturedDtos.get(0);
    }

    assertEquals(scheduleDto1.getId(), schedule1Id);
    assertEquals(scheduleDto2.getId(), schedule2Id);
    ...
    assertEquals(scheduleDto1.getScheduledTime(), schedule1.getScheduledTime());
    assertEquals(scheduleDto2.getScheduledTime(), schedule2.getScheduledTime());
